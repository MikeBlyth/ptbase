# Based on Railscasts episode 311, by Ryan Bates
# LIMITATION: tag ID is formed from name, so will not be unique if the same name is used
...
class LabeledFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def field_label(name, *args)
    ...
    label(name, options[:label], class: ("required" if required), for: "#{name}_id")
  end
